...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="modellering-nytta-epub-") as tmp_name:
        tmp = Path(tmp_name)
        ...
        text = re.sub(r'(<itemref\b[^>]*idref="nav"[^>]*)(/?>)', lambda m: m.group(1) if 'linear=' in m.group(1) else m.group(1) + ' linear="no"' + m.group(2), text)
        ...
        with zipfile.ZipFile(rebuilt, "w") as archive:
            ...
            if mimetype.exists(): archive.write(mimetype, "mimetype", compress_type=zipfile.ZIP_STORED)
...
def main() -> int:
    ...
    if not metadata_path.exists(): metadata_path = root / "book.yaml"
    ...
    if shutil.which("pandoc") is None: raise SystemExit("Pandoc saknas")
    version = pandoc_version()
    if version != PANDOC_VERSION: raise SystemExit(f"Pandoc {PANDOC_VERSION} krävs; hittade {version}")
    ...
    run(["pandoc", *map(str, chapters), "--from=markdown", "--to=epub3", "--output", str(epub), "--metadata-file", str(metadata_path), "--resource-path", resource_path, "--lua-filter", str(chapter_filter), "--toc", "--toc-depth=1", "--css", str(root / "publishing" / "epub.css"), "--epub-cover-image", str(root / metadata["cover_image"])], root)
    postprocess_epub(epub)
    if shutil.which("xelatex") is None: raise SystemExit("xelatex krävs för PDF-bygget")
    pdf = output_dir / f"{slug}.pdf"
    with tempfile.TemporaryDirectory(prefix="modellering-nytta-pdf-") as tmp_name:
        tmp = Path(tmp_name); front = tmp / "frontmatter.tex"; tex = tmp / "book.tex"
        title = latex_escape(str(metadata.get("title", ""))); subtitle = latex_escape(str(metadata.get("subtitle", ""))); author = latex_escape(str(metadata.get("author", ""))); cover = (root / metadata["cover_image"]).as_posix()
        front.write_text("\\pagenumbering{gobble}\n\\thispagestyle{empty}\n" + f"\\AddToShipoutPictureBG*{{\\AtPageLowerLeft{{\\includegraphics[width=\\paperwidth,height=\\paperheight]{{{cover}}}}}}}\n" + "\\null\\clearpage\n\\thispagestyle{empty}\n\\vspace*{0.22\\textheight}\n\\begin{center}\n" + f"{{\\Huge\\bfseries {title}}}\\par\n" + f"\\vspace{{1em}}{{\\Large {subtitle}}}\\par\n" + "\\vfill\n" + f"{{\\Large {author}}}\\par\n" + "\\end{center}\\clearpage\n\\pagenumbering{roman}\n\\phantomsection\n\\pdfbookmark[1]{Innehåll}{toc}\n\\tableofcontents\n\\clearpage\n", encoding="utf-8")
        run(["pandoc", *map(str, chapters), "--from=markdown+raw_tex+pipe_tables", "--to=latex", "--standalone", "--output", str(tex), "--resource-path", resource_path, "--lua-filter", str(chapter_filter), "--include-in-header", str(root / "publishing" / "pdf-header.tex"), "--include-before-body", str(front), "--metadata", "title="], root)
        xelatex = ["xelatex", "-interaction=nonstopmode", "-halt-on-error", f"-output-directory={tmp}", f"-jobname={slug}", str(tex)]
        for _ in range(3): run(xelatex, root)
        built_pdf = tmp / f"{slug}.pdf"
        if not built_pdf.exists(): raise SystemExit("PDF-bygget skapade ingen utfil")
        shutil.copy2(built_pdf, pdf)
    print(f"OK: {epub}"); print(f"OK: {pdf}"); return 0
